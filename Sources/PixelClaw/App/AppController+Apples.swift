...
extension AppController {
    ...
    func reactToTopAppleHit(from apple: AppleState) {
        ...
        if abs(apple.x - crabX) < 3 {
            if level == .dock {
                ...
                direction = crabView.facingRight ? -1 : 1
            }
        } else {
    ...
    func crabHitRect() -> CGRect {
        ...
        let ox: CGFloat = 10 * s
        let oy: CGFloat = 4 * s
        let legRows = CGFloat(crabView.currentLegs.count)
        let bottom = oy - legRows * s
        let top = oy + CGFloat(bodyGrid.count) * s + crabView.bodyBob + crabView.legYBob
        return CGRect(
            x: crabView.frame.origin.x + ox,
            y: crabView.frame.origin.y + bottom,
            width: 10 * s,
